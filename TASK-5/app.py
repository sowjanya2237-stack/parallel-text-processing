...
st.markdown("""
    ...
    div.stButton > button:first-child, div.stDownloadButton > button:first-child { 
        ...
        padding: 0.8rem !important; font-weight: 800 !important; 
...
if page == "File Portal":
    ...
    if uploaded_file is not None:
        if st.session_state.uploaded_data_list is None:
            with st.spinner("Analyzing dataset structure (Comma Default)..."):
...
# --- PAGE 2: PROCESS ENGINE (Updated to match screenshots) ---
    ...
    if st.session_state.uploaded_data_list:
        ...
        # RUN button matching white gradient requirement
        if st.button("RUN"):
            start_time = time.time()
            c_size = 50000 
            chunks = [data[i:i + c_size] for i in range(0, len(data), c_size)]
            ...
            with st.status("processing...", expanded=True) as status:
                ...
                # Combine data: (text, score, sentiment, timestamp)
                final_data = [(data[i], flat_results[i][0], flat_results[i][1], flat_results[i][2]) for i in range(len(data))]
                ...
                cursor.executemany("INSERT INTO results VALUES (?, ?, ?, ?)", final_data)
                conn.commit()
                conn.close()
                status.update(label="Completed", state="complete")
            
            duration = time.time() - start_time
            # Green success message matching screenshot
            st.success(f"PROCESSED {len(final_data):,} RECORDS IN {duration:.2f}s")
            
            # Result Matrix preview
            st.dataframe(pd.DataFrame(final_data[:50], columns=["text", "score", "sentiment", "timestamp"]), use_container_width=True)
    else:
...
# --- PAGE 3: REGISTRY VAULT ---
    ...
    if os.path.exists(DB_NAME):
        ...
        st.markdown(f"### Registry Matrix ({f_choice})"); st.dataframe(db_df, use_container_width=True, height=400)
        ...
        if not receiver_email: st.error("Provide email.")
            ...
            with st.status("Compiling Report...", expanded=True) as status:
                try:
                    ...
                    p_csv = MIMEBase('application', 'octet-stream'); p_csv.set_payload(df_sample_att.to_csv(index=False).encode('utf-8')); encoders.encode_base64(p_csv); p_csv.add_header('Content-Disposition', 'attachment; filename="review_database.csv"'); msg.attach(p_csv)

                    srv = smtplib.SMTP('smtp.gmail.com', 587); srv.starttls(); srv.login(SENDER_EMAIL, SENDER_PASSWORD)
                    srv.send_message(msg); srv.quit()
                    status.update(label="Dispatch Successful!", state="complete"); st.success(f"Report dispatched to {receiver_email}")
