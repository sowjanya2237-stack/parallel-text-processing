        ...
        if st.session_state.uploaded_data_list is None:
            with st.spinner("Analyzing..."):
                try:
                    ...
                    target_col = st.selectbox("Select Text Column to Analyze", cols, index=0)
    ...
    if st.session_state.uploaded_data_list:
        ...
        if st.button("RUN"):
            ...
            with st.status("processing...", expanded=True) as status:
                ...
                conn.commit(); conn.close(); status.update(label="Completed", state="complete")
...
# PAGE 3: REGISTRY VAULT 
    ...
    # 1. THE SYSTEM HEARTBEAT CHECK
    db_exists = os.path.exists(DB_NAME)
    registry_status = "OFFLINE"
    total_rec = 0
    db_df_preview = pd.DataFrame()
    ...
    with m2: 
        st.metric("REGISTRY STATUS", registry_status)
        if registry_status != "ACTIVE":
            st.error(f"System Alert: Database connection is {registry_status}")
            
    with m3: 
        st.metric("ENCRYPTION", "VERIFIED" if registry_status == "ACTIVE" else "UNAVAILABLE")
    ...
    if registry_status == "ACTIVE":
        ...
        st.warning("⚠️ Access Denied: The Registry Vault is currently offline or uninitialized. Please run the Process Engine to establish a connection.")
    ...
    if os.path.exists(DB_NAME):
        if st.button("PREPARE ENTIRE VAULT FOR DOWNLOAD"):
            with st.status("PREPARING...", expanded=True) as status:
                ...
                st.download_button("📥 DOWNLOAD DATABASE", csv_bytes, "CORE_VAULT_EXPORT.csv", "text/csv")
        ...
        if not receiver_email: st.error("Provide email.")
            ...
            with st.status("Compiling Report...", expanded=True) as status:
                try:
                    ...
                    # Fetch real samples for the email
                    ...
                    # Fetch sample dataset (Latest 1,000 records) for attachment
                    df_sample_att = pd.read_sql_query("SELECT * FROM results ORDER BY rowid DESC LIMIT 1000", conn)
                    conn.close()
                    
                    p_c, n_c, nu_c = c_dict.get('Positive', 0), c_dict.get('Negative', 0), c_dict.get('Neutral', 0)
                    p_p, n_p, nu_p = (p_c/total*100), (n_c/total*100), (nu_c/total*100) if total > 0 else (0,0,0)
...
The complete line-by-line scoring database is available for download"""
                    ...
                    p_csv = MIMEBase('application', 'octet-stream'); p_csv.set_payload(df_sample_att.to_csv(index=False).encode('utf-8')); encoders.encode_base64(p_csv)
                    p_csv.add_header('Content-Disposition', 'attachment; filename="sample_dataset.csv"'); msg.attach(p_csv)

                    # Send
                    srv = smtplib.SMTP('smtp.gmail.com', 587); srv.starttls(); srv.login(SENDER_EMAIL, SENDER_PASSWORD)
                    srv.send_message(msg); srv.quit()
                    status.update(label="Report Dispatched Successfully!", state="complete"); st.success(f"Sent to {receiver_email}")
